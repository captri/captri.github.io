...
Empezamos haciendo las siguientes dos suposiciones para solventar problemas iniciales: 
		\begin{itemize}
			\item [i)] Si $x$ es un punto periódico, de período $k\leq 2m+1$, de $\varphi$ los autovalores de $(d\varphi^k)_x$ son diferentes y ninguno es 1.
			\item [ii)] Puntos fijos de $\varphi$ no están en el mismo nivel bajo $f$, es decir, si $x$, $y$ son puntos fijos: $f(x)\neq f(y)$  
		\end{itemize}
